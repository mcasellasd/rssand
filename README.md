...
- `APP_TIME_ZONE`: zona horària de la programació diària; per defecte `Europe/Andorra`.
- `DAILY_SUMMARY_STORE_PATH`: fitxer opcional on conservar els darrers resums diaris; per defecte `.runtime/daily-summaries.json`.
- `WEEKLY_BRIEF_USAGE_PATH`: fitxer opcional per conservar el consum diari del briefing; per defecte `.runtime/weekly-brief-usage.json`.
...
## Briefing diari i límit de generació

El briefing setmanal només es pot generar una vegada per dia natural (hora d’Andorra). Les consultes del resultat ja generat continuen disponibles i les noves generacions es tornen a habilitar l’endemà.

El servidor prepara automàticament, cada dia a les 08:00, el resum de les publicacions del dia anterior. El resum queda disponible a `/api/news/daily-summary` i es mostra a la targeta `Resum diari` de la interfície. Si el procés es reinicia després de les 08:00, la primera consulta recupera el resum pendent i el genera.
