...
const APP_TIME_ZONE = process.env.APP_TIME_ZONE || 'Europe/Andorra';
const DAILY_BRIEFING_LIMIT = 1;
const DAILY_SUMMARY_STORE_PATH = process.env.DAILY_SUMMARY_STORE_PATH
  || path.join(__dirname, '.runtime', 'daily-summaries.json');
const WEEKLY_BRIEF_USAGE_PATH = process.env.WEEKLY_BRIEF_USAGE_PATH
  || path.join(__dirname, '.runtime', 'weekly-brief-usage.json');
const dailySummaryCache = new Map();
const dailySummaryInFlight = new Map();
const weeklyBriefGenerationState = new Map();
let dailySummaryStoreLoaded = false;
let weeklyBriefUsageLoaded = false;
...
function getZonedDateParts(date = new Date(), timeZone = APP_TIME_ZONE) {
...
function getNextLocalMidnightDateKey(date = new Date()) {
  return shiftDateKey(getLocalDateKey(date), 1);
}

function loadDailySummaryStore() {
  if (dailySummaryStoreLoaded) return;
  dailySummaryStoreLoaded = true;
  try {
    const stored = JSON.parse(fs.readFileSync(DAILY_SUMMARY_STORE_PATH, 'utf8'));
    if (!stored || typeof stored !== 'object') return;
    Object.entries(stored).forEach(([dateKey, summary]) => {
      if (/^\d{4}-\d{2}-\d{2}$/.test(dateKey) && summary && typeof summary === 'object') {
        dailySummaryCache.set(dateKey, summary);
      }
    });
  } catch (error) {
    if (error.code !== 'ENOENT') console.error('No s’ha pogut llegir el resum diari desat:', error.message);
  }
}

function persistDailySummary(dateKey, summary) {
  loadDailySummaryStore();
  dailySummaryCache.set(dateKey, summary);
  const entries = [...dailySummaryCache.entries()]
    .sort(([dateA], [dateB]) => dateB.localeCompare(dateA))
    .slice(0, 14);
  const payload = Object.fromEntries(entries);
  const directory = path.dirname(DAILY_SUMMARY_STORE_PATH);
  const temporaryPath = `${DAILY_SUMMARY_STORE_PATH}.tmp`;
  try {
    fs.mkdirSync(directory, { recursive: true });
    fs.writeFileSync(temporaryPath, JSON.stringify(payload, null, 2));
    fs.renameSync(temporaryPath, DAILY_SUMMARY_STORE_PATH);
  } catch (error) {
    console.error('No s’ha pogut desar el resum diari:', error.message);
    try { fs.unlinkSync(temporaryPath); } catch (cleanupError) { /* best effort */ }
  }
}

function getLatestDailySummary() {
  loadDailySummaryStore();
  return [...dailySummaryCache.entries()]
    .sort(([dateA], [dateB]) => dateB.localeCompare(dateA))[0] || null;
}

function loadWeeklyBriefUsage() {
  if (weeklyBriefUsageLoaded) return;
  weeklyBriefUsageLoaded = true;
  try {
    const stored = JSON.parse(fs.readFileSync(WEEKLY_BRIEF_USAGE_PATH, 'utf8'));
    if (!stored || typeof stored !== 'object') return;
    Object.entries(stored).forEach(([dateKey, state]) => {
      if (/^\d{4}-\d{2}-\d{2}$/.test(dateKey) && state && typeof state === 'object') {
        weeklyBriefGenerationState.set(dateKey, {
          completed: Math.min(Number(state.completed) || 0, DAILY_BRIEFING_LIMIT),
          inFlight: false
        });
      }
    });
  } catch (error) {
    if (error.code !== 'ENOENT') console.error('No s’ha pogut llegir el límit diari del briefing:', error.message);
  }
}

function persistWeeklyBriefUsage() {
  const entries = [...weeklyBriefGenerationState.entries()]
    .sort(([dateA], [dateB]) => dateB.localeCompare(dateA))
    .slice(0, 14);
  const directory = path.dirname(WEEKLY_BRIEF_USAGE_PATH);
  const temporaryPath = `${WEEKLY_BRIEF_USAGE_PATH}.tmp`;
  try {
    fs.mkdirSync(directory, { recursive: true });
    fs.writeFileSync(temporaryPath, JSON.stringify(Object.fromEntries(entries), null, 2));
    fs.renameSync(temporaryPath, WEEKLY_BRIEF_USAGE_PATH);
  } catch (error) {
    console.error('No s’ha pogut desar el límit diari del briefing:', error.message);
    try { fs.unlinkSync(temporaryPath); } catch (cleanupError) { /* best effort */ }
  }
}

function beginWeeklyBriefGeneration(date = new Date()) {
  loadWeeklyBriefUsage();
  const dateKey = getLocalDateKey(date);
  const state = weeklyBriefGenerationState.get(dateKey) || { completed: 0, inFlight: false };
  if (state.completed >= DAILY_BRIEFING_LIMIT || state.inFlight) {
    const error = new Error('El briefing setmanal ja s’ha generat avui. Torna-ho a provar demà.');
    error.status = 429;
    error.code = 'DAILY_LIMIT_REACHED';
    error.nextAllowedDate = getNextLocalMidnightDateKey(date);
    throw error;
  }
  // Reserve the quota before calling an external model. This prevents two
  // concurrent requests and keeps the daily cap after a process restart.
  state.completed += 1;
  state.inFlight = true;
  weeklyBriefGenerationState.set(dateKey, state);
  persistWeeklyBriefUsage();
  return dateKey;
}

function finishWeeklyBriefGeneration(dateKey) {
  const state = weeklyBriefGenerationState.get(dateKey);
  if (!state) return;
...
function getWeeklyBriefGenerationStatus(date = new Date()) {
  loadWeeklyBriefUsage();
  const dateKey = getLocalDateKey(date);
  const state = weeklyBriefGenerationState.get(dateKey) || { completed: 0, inFlight: false };
  return {
    ...
    used: state.completed,
    available: state.completed < DAILY_BRIEFING_LIMIT && !state.inFlight,
    nextAllowedDate: state.completed >= DAILY_BRIEFING_LIMIT ? getNextLocalMidnightDateKey(date) : null
  };
}
...
function getLegalRelevance(title = '', category = '') {
  ...
  if (officialCategories.includes(category) || category.startsWith('Comú de')) return 'medium';
...
function buildDeterministicDailySummary(dailyNews, newsForPrompt, targetDate, reason = null) {
  const baseSummary = buildDeterministicSummary(dailyNews, newsForPrompt, reason, 'all');
  const dateLabel = parseDateToCatalan(targetDate).toLowerCase();
  const countBy = field => dailyNews.reduce((counts, item) => {
    const value = item[field] || 'General';
    counts[value] = (counts[value] || 0) + 1;
    return counts;
  }, {});
  const topEntries = counts => Object.entries(counts)
    .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0], 'ca'));
  const areasText = topEntries(countBy('practiceArea')).slice(0, 4)
    .map(([name, count]) => `${name} (${count})`).join(', ');
  const typesText = topEntries(countBy('documentType')).slice(0, 3)
    .map(([name, count]) => `${name} (${count})`).join(', ');
  return {
    ...baseSummary,
    period: 'daily',
    targetDate,
    timestamp: new Date().toISOString(),
    resumExecutiu: dailyNews.length
      ? `El ${dateLabel} s’han identificat ${dailyNews.length} publicacions oficials amb rellevància jurídica. Les àrees amb més activitat són ${areasText || 'les àrees generals i institucionals'} i els tipus documentals destacats són ${typesText || 'les publicacions oficials generals'}.`
      : `El ${dateLabel} no s’han identificat publicacions oficials amb rellevància jurídica en les fonts consultades.`,
    editorialNote: reason
      ? 'Resum diari automàtic basat exclusivament en metadades oficials; la capa editorial d’IA no estava disponible.'
      : 'Resum diari automàtic basat exclusivament en metadades oficials.'
  };
}
...
function getProfessionalReview(item = {}) {
  ...
  const reviewByType = {
    ...
    'Aprovació parlamentària': {
      ...
      professionalAction: 'Comprovar la publicació oficial, el text definitiu i la data d’entrada en vigor.'
    },
    'Llei': {
      legalStage: item.officialDocument ? 'Publicat oficialment' : 'Seguiment normatiu',
...
function extractDocumentSignals(html = '') {
...
async function getAiSummary(forceRefresh = false, requestedPracticeArea = 'all') {
  ...
  const generationDateKey = beginWeeklyBriefGeneration();
  ...
  if (weeklyNews.length === 0) {
    ...
    finishWeeklyBriefGeneration(generationDateKey);
    return emptySummary;
  }
...
async function generateDailySummaryForDate(targetDate) {
  let items = newsCache;
  if (!items || !cacheTimestamp || (Date.now() - cacheTimestamp > CACHE_DURATION)) {
    items = await fetchAllFeeds();
  }

  const dailyNews = (items || []).filter(item =>
    item.date === targetDate &&
    item.legalRelevance !== 'low' &&
    item.isLegislative !== false
  );
  const newsForPrompt = selectEditorialItems(dailyNews, 18);

  if (dailyNews.length === 0) {
    return buildDeterministicDailySummary([], [], targetDate);
  }

  const fallbackSummary = buildDeterministicDailySummary(dailyNews, newsForPrompt, targetDate);
  if (!ai) {
    return buildDeterministicDailySummary(dailyNews, newsForPrompt, targetDate, 'missing_api_key');
  }

  const newsSummaryText = newsForPrompt.map((item, index) =>
    `[${index + 1}] Font: ${item.source} | Data: ${item.date} | Tipus: ${item.documentType} | Àrea: ${item.practiceArea}\nTítol: ${item.title}\nDescripció: ${item.snippet || ''}\nEnllaç: ${item.link || ''}`
  ).join('\n---\n');
  const prompt = `Ets l’editor jurídic d’un servei d’actualitat professional del Principat d’Andorra.
A partir exclusivament de les publicacions oficials del dia ${targetDate}, redacta en català un resum diari rigorós i concís del dia anterior.

Publicacions del dia:
${newsSummaryText}

No inventis dades, terminis, efectes jurídics ni conclusions. Conserva exactament els títols i enllaços originals a "noticiesAmbImpacte". Explica breument què convé revisar, sense donar assessorament jurídic formal.`;
  ...
  try {
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            puntsClau: { type: 'ARRAY', items: { type: 'STRING' } },
            categoriesDestacades: {
              type: 'ARRAY',
              items: {
                type: 'OBJECT',
    ...
    const generated = normalizeAiSummary(JSON.parse(response.text), newsForPrompt, fallbackSummary);
    return {
      ...generated,
      period: 'daily',
      targetDate,
    ...
    console.error('Resum diari IA no disponible; s’utilitza la síntesi automàtica:', error.message);
    return buildDeterministicDailySummary(dailyNews, newsForPrompt, targetDate, error.message);
  }
}

async function getDailySummaryForDate(targetDate) {
  loadDailySummaryStore();
  if (dailySummaryCache.has(targetDate)) return dailySummaryCache.get(targetDate);
  if (dailySummaryInFlight.has(targetDate)) return dailySummaryInFlight.get(targetDate);

  const promise = generateDailySummaryForDate(targetDate)
    .then(summary => {
      persistDailySummary(targetDate, summary);
      return summary;
    })
    .finally(() => dailySummaryInFlight.delete(targetDate));
  dailySummaryInFlight.set(targetDate, promise);
  return promise;
}
...
async function runDailySummaryIfDue(now = new Date()) {
  ...
  const targetDate = shiftDateKey(`${parts.year}-${parts.month}-${parts.day}`, -1);
  return getDailySummaryForDate(targetDate);
}

function millisecondsUntilNextDailyRun(now = new Date()) {
  const firstMinute = new Date(now);
  firstMinute.setSeconds(0, 0);
  if (firstMinute.getTime() <= now.getTime()) firstMinute.setMinutes(firstMinute.getMinutes() + 1);
  for (let minutes = 0; minutes <= 48 * 60; minutes += 1) {
    const candidate = new Date(firstMinute.getTime() + minutes * 60 * 1000);
    const parts = getZonedDateParts(candidate);
    if (Number(parts.hour) === 8 && Number(parts.minute) === 0) {
...
function scheduleDailySummary() {
  runDailySummaryIfDue().catch(error => console.error('Error en el resum diari programat:', error.message));
  const delay = millisecondsUntilNextDailyRun();
  setTimeout(() => scheduleDailySummary(), delay);
}
...
app.get('/api/news/summary', async (req, res) => {
  try {
    ...
    res.json({ ...summary, generationLimit: getWeeklyBriefGenerationStatus() });
  } catch (error) {
    if (error.status !== 400) {
    ...
    res.status(status).json({
      error: error.code === 'DAILY_LIMIT_REACHED' ? 'DAILY_LIMIT_REACHED' : errorMsg,
      message: error.code === 'DAILY_LIMIT_REACHED' ? error.message : undefined,
      nextAllowedDate: error.nextAllowedDate || undefined
    });
  }
});

app.get('/api/news/daily-summary', async (req, res) => {
  res.set('Cache-Control', 'no-store');
  const now = new Date();
  const parts = getZonedDateParts(now);
  const currentDate = `${parts.year}-${parts.month}-${parts.day}`;
  const targetDate = shiftDateKey(currentDate, -1);
  const nextRun = `${currentDate}T08:00:00`;

  try {
    loadDailySummaryStore();
    let summary = dailySummaryCache.get(targetDate);
    const afterDailyRun = Number(parts.hour) >= 8;
    if (!summary && afterDailyRun) summary = await getDailySummaryForDate(targetDate);
    ...
    return res.json({
      ...
      targetDate: latest ? latest[0] : targetDate,
      generatedAt: latest ? latest[1].timestamp : null,
      summary: latest ? latest[1] : null,
      waitingFor: targetDate,
      scheduledAt: nextRun,
      scheduledTime: '08:00',
      timeZone: APP_TIME_ZONE,
      message: `El resum del ${targetDate} es prepararà a les 08:00 (hora d’Andorra).`
    });
  } catch (error) {
    console.error('Error servint el resum diari:', error.message);
    res.status(500).json({ error: 'No s’ha pogut carregar el resum diari.' });
  }
});
...
app.get('/api/news/newsletter', async (req, res) => {
  try {
    ...
    res.json({
      ...
      itemsCount: weeklyNews.length,
      practiceArea,
      generationLimit: getWeeklyBriefGenerationStatus(),
      data: newsletterData,
      html: htmlContent,
      text: textContent
    });
...
module.exports = {
  ...
  getWeeklyBriefGenerationStatus,
