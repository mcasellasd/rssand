document.addEventListener('DOMContentLoaded', () => {
    ...
    const dailySummaryEmpty = document.getElementById('daily-summary-empty');
    ...
    function applyFiltersAndRender() {
        ...
        if (currentTab === 'newsletter') {
            ...
            feedSkeletons.classList.add('hide');
            feedEmpty.classList.add('hide');
            dailySummaryContainer.classList.add('hide');
            ...
            dailySummaryContainer.classList.remove('hide');
    ...
    function createNewsCard(item) {
        ...
        card.innerHTML = `
            <div>
                ...
                <h3 class="news-title">
                    <a href="${safeLink}" target="_blank" rel="noopener noreferrer" title="Obrir la notícia">${escapeHtml(item.title)}</a>
                </h3>
                
                ${entryIntoForceHtml}
                ${operativeDeadlinesHtml}
                
                <p class="news-snippet">${escapeHtml(item.snippet || "Sense descripció disponible.")}</p>
            ...
            <div class="card-footer">
                <div class="card-tags">
                    <span class="category-tag">
                        <i class="fa-solid fa-file-lines"></i> ${escapeHtml(item.documentType || "Actualitat")}
                    </span>
                ...
                <div class="card-actions">
                    ...
                    <a href="${safeLink}" target="_blank" rel="noopener noreferrer" class="btn-action-link" title="Consultar la font">
                        Font <i class="fa-solid fa-arrow-up-right-from-square"></i>
                    </a>
                </div>
            </div>
        `;
    ...
    function apiErrorMessage(errorData, fallback) {
        if (errorData?.error === 'DAILY_LIMIT_REACHED') {
            return 'El briefing setmanal ja s’ha generat avui. Torna-ho a provar demà.';
        }
        return errorData?.message || errorData?.error || fallback;
    }

    function renderDailySummary(data) {
        dailySummaryLoading.classList.add('hide');
        dailySummaryEmpty.classList.add('hide');
        dailySummaryContent.classList.add('hide');

        if (!data.available || !data.summary) {
            dailySummaryStatus.textContent = 'Programat cada dia a les 08:00';
            dailySummaryEmpty.textContent = data.message || 'El resum diari encara no està disponible.';
            dailySummaryEmpty.classList.remove('hide');
            return;
        }

        const summary = data.summary;
        dailySummaryStatus.textContent = `Dia anterior · ${formatDateDisplay(data.targetDate)}`;
        dailySummaryExecutive.textContent = summary.resumExecutiu || 'Sense resum executiu disponible.';
        dailySummaryPoints.innerHTML = '';
        (summary.puntsClau || []).forEach(point => {
            const li = document.createElement('li');
            li.textContent = point;
            dailySummaryPoints.appendChild(li);
        });
        if (!dailySummaryPoints.children.length) {
            dailySummaryPoints.innerHTML = '<li>No hi ha punts clau destacats.</li>';
        }

        dailySummaryLinks.innerHTML = '';
        (summary.noticiesAmbImpacte || []).forEach(item => {
            const link = document.createElement('a');
            link.className = 'daily-summary-link';
        ...
        dailySummaryMethod.title = summary.editorialNote || '';
        const generatedAt = summary.timestamp ? new Date(summary.timestamp) : null;
        const generatedText = generatedAt && !Number.isNaN(generatedAt.getTime())
            ? generatedAt.toLocaleTimeString('ca-ES', { hour: '2-digit', minute: '2-digit' })
            : '';
        dailySummaryDate.textContent = generatedText ? `Generat a les ${generatedText}` : '';
        dailySummaryContent.classList.remove('hide');
    }

    async function loadDailySummary() {
        try {
            const response = await fetch('/api/news/daily-summary');
            const data = await response.json();
            if (!response.ok) throw new Error(apiErrorMessage(data, 'No s’ha pogut carregar el resum diari.'));
            renderDailySummary(data);
        } catch (error) {
            console.error('Error carregant el resum diari:', error);
            dailySummaryLoading.classList.add('hide');
            dailySummaryStatus.textContent = 'Resum diari no disponible';
            dailySummaryEmpty.textContent = error.message;
            dailySummaryEmpty.classList.remove('hide');
        }
    }
    ...
    async function loadAiSummary() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(apiErrorMessage(errData, `Error del servidor: ${response.status}`));
            }
    ...
    // INITIAL LOAD
    loadNewsFeed().then(() => loadDailySummary());
    // Pick up the scheduled 08:00 summary while the app remains open.
    setInterval(loadDailySummary, 5 * 60 * 1000);
